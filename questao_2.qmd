...
```{r}
X_prev_browniano <- simulate_Xtk(t, stock_values, estim_theta_browniano, B)
X_prev_poisson <- simulate_Xtk(t, stock_values, estim_theta_poisson, N)
...
```{r}
dados_sim |> ggplot() +
    geom_line(aes(x = t, y = X, color = "X")) +
    geom_line(aes(x = t, y = X_prev_browniano, color = "X Previsto Browniano")) +
    labs(x = "Tempo", y = "Valor das ações do BBAS3", title = "Ajuste do modelo de movimento Browniano aos dados de ações do BBAS3 em 2023") +
    theme_minimal()
```
...
dados_sim |> ggplot() +
    geom_line(aes(x = t, y = X, color = "X")) +
    geom_line(aes(x = t, y = X_prev_poisson, color = "X Previsto Poisson")) +
    labs(x = "Tempo", y = "Valor das ações do BBAS3", title = "Ajuste do modelo de processos de Poisson aos dados de ações do BBAS3 em 2023") +
    theme_minimal()
```
...
N_valid <- c(0, cumsum(rpois(n_valid, 1 / n_valid)))
```


```{r}
X_prev_valid <- simulate_Xtk(t_valid, stock_values_valid, estim_theta_poisson, N_valid)

dados_valid <- data.frame(
    t = t_valid[-1],
    X = stock_values_valid[-1],
    X_prev_valid = X_prev_valid
)
```
...
dados_valid |> ggplot() +
    geom_line(aes(x = t, y = X, color = "X")) +
    geom_line(aes(x = t, y = X_prev_valid, color = "X Previsto Poisson")) +
    labs(x = "Tempo", y = "Valor das ações do BBAS3", title = "Ajuste do modelo de processos de Poisson aos dados de ações do BBAS3 nas primeiras semanas de 2024") +
    theme_minimal()
```
